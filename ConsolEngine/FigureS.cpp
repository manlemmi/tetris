...
FigureS::FigureS(int x, int y) : Tetromino(x, y), secondTurn(false)
{
	...
	currentCoords.at(3).reset(new SingleBrick(x - 1, y + 1));
	...
	prevCoords.at(3).reset(new SingleBrick(x - 1, y + 1));
}
